...
export function SprintCard({ sprint, isSelected, onSelect, onDelete }: SprintCardProps) {
  ...
  return (
    <Card className={`cursor-pointer transition-all hover:shadow-md ${isSelected ? "ring-2 ring-primary" : ""}`}>
      ...
      <CardContent onClick={onSelect}>
        ...
        <div className="mt-2 text-xs text-muted-foreground">Duration: {endDate.diff(startDate, "day")} days</div>
      </CardContent>
    </Card>
  )
}
